...
const productionState = { user:null, permissions:new Set(), products:[], lines:[], categories:[], sectors:[], records:[], stops:[], currentShift:null, previousProductId:null, editingClosed:false, originalShiftData:null, querySort:{key:null,direction:"asc"}, stopSort:{key:null,direction:"asc"}, visibleProductionRows:[], visibleStopRows:[] };
...
function syncProductionEndTimes({onlyMissing=false}={}){
...
async function checkShiftStatus(){
  ...
  if(closed){const[productions,stops]=await Promise.all([window.LIDUTEC_PRODUCAO_DATA.shiftProductions(data.id),window.LIDUTEC_PRODUCAO_DATA.shiftStops(data.id)]);productionState.originalShiftData={productions,stops};populateShiftRows(productions,stops);syncProductionEndTimes({onlyMissing:true});form.dataset.loadedClosedShift=`${date}|${shift}`}else{productionState.originalShiftData=null;if(form.dataset.loadedClosedShift){resetShiftEntryRows();delete form.dataset.loadedClosedShift}}
