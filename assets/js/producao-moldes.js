...
function calendarDayState(value,shift){const turn=shiftCalendarState.turns.get(value),bounds=window.LIDUTEC_TURNOS.shiftBounds(value,shift),now=new Date(),events=shiftCalendarState.events.filter(event=>event.data_inicio<=value&&event.data_fim>=value),work=events.find(event=>event.tipo==="TRABALHO_EXCEPCIONAL"),holiday=events.find(event=>event.tipo==="FERIADO"),ferias=events.find(event=>event.tipo==="FERIAS_COLETIVAS"),folga=events.find(event=>event.tipo==="FOLGA_PROGRAMADA");if(turn?.status==="FECHADO"){if(holiday)return{type:"worked-holiday",label:`Trabalhado (feriado) · ${holiday.nome}`};return{type:"worked",label:`Trabalhado${events.length?` · ${events.map(event=>event.nome).join(" · ")}`:""}`}}if(turn?.status==="ABERTO")return{type:"incomplete",label:"Turno com dados faltando finalizar"};if(bounds.end>now)return{type:"off",label:"Futuro"};if(work)return{type:"missing",label:`Trabalho excepcional sem apontamento · ${work.nome}`};if(holiday)return{type:"holiday",label:holiday.nome};if(ferias)return{type:"vacation",label:ferias.nome};if(folga)return{type:"off",label:folga.nome};if(!window.LIDUTEC_TURNOS.isScheduledShiftDay(value,shift))return{type:"off",label:"Folga"};return{type:"missing",label:"Turno previsto sem apontamento"}}
