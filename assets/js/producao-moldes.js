...
const productionState = { user:null, permissions:new Set(), products:[], lines:[], categories:[], sectors:[], records:[], stops:[], currentShift:null, previousProductId:null, editingClosed:false, originalShiftData:null, querySort:{key:null,direction:"asc"}, stopSort:{key:null,direction:"asc"} };
...
function syncProductionEndTimes(){
  if(productionState.editingClosed)return;
  const bounds=shiftDateTimeBounds(),rows=[...document.querySelectorAll(".shift-production-row")];if(!bounds||!rows.length)return;
  const now=Math.min(Date.now(),bounds.end.getTime()),format=value=>{const date=new Date(value),pad=item=>String(item).padStart(2,"0");return`${pad(date.getHours())}:${pad(date.getMinutes())}`};
  rows.forEach((row,index)=>{const startInput=row.querySelector('[name="inicio"]'),endInput=row.querySelector('[name="fim"]'),start=resolveShiftTime(startInput.value)?.getTime()??NaN,nextValue=rows[index+1]?.querySelector('[name="inicio"]')?.value,nextStart=nextValue?resolveShiftTime(nextValue)?.getTime()??NaN:NaN,candidate=Number.isNaN(nextStart)?now:nextStart-60000;if(!startInput.value||Number.isNaN(start)||candidate<=start||candidate>bounds.end.getTime()){endInput.value="";syncRequiredFields(row,["inicio","produto_id","moldes_vazados","moldes_quebrados"]);return}endInput.value=format(candidate);endInput.title=Number.isNaN(nextStart)?"Atualizado até o horário atual, respeitando o limite do turno.":"Calculado como o início da próxima linha menos 1 minuto.";syncRequiredFields(row,["inicio","produto_id","moldes_vazados","moldes_quebrados"])});
  renderShiftTimeline();
}
...
async function checkShiftStatus(){
  ...
  const bounds=window.LIDUTEC_TURNOS.shiftBounds(date,shift),previousMoment=new Date(bounds.start.getTime()-60000),previousShift=window.LIDUTEC_TURNOS.determineShift(previousMoment),previousBounds=window.LIDUTEC_TURNOS.shiftBounds(previousShift.dataOperacional,previousShift.codigo),[data,previous]=await Promise.all([window.LIDUTEC_PRODUCAO_DATA.shift(date,shift),window.LIDUTEC_PRODUCAO_DATA.previousProduction(previousBounds.start.toISOString(),previousBounds.end.toISOString())]);productionState.previousProductId=previous?.produto_id??null;for(const row of document.querySelectorAll(".shift-production-row"))updateProductionRow(row);document.dispatchEvent(new CustomEvent("production-setup-context-changed"));
...
function initializeShiftEntry(){
  ...
  form.elements.data_operacional.addEventListener("change",()=>{applyShiftDateTimeLimits();syncProductionEndTimes();renderShiftTimeline();saveShiftDraft();checkShiftStatus().catch(error=>message(error.message,"error"))});form.elements.turno.addEventListener("change",()=>{applyShiftDateTimeLimits();syncProductionEndTimes();renderShiftTimeline();saveShiftDraft();checkShiftStatus().catch(error=>message(error.message,"error"))});form.addEventListener("submit",closeShift);applyShiftDateTimeLimits();syncProductionEndTimes();renderShiftTimeline();checkShiftStatus().catch(error=>message(error.message,"error"));setInterval(()=>{if(!document.hidden)syncProductionEndTimes()},60000);
