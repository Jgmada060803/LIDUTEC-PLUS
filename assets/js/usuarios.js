...
(async()=>{const user=await window.LIDUTEC_APP.requireAuthenticatedUser();if(!user)return;const [profile,p]=await Promise.all([window.LIDUTEC_APP.getCurrentUserProfile(user.id),window.LIDUTEC_APP.getUserPermissions(user.id)]);permissions=p;if(!p.has("usuarios.visualizar"))return location.replace("../dashboard.html");window.LIDUTEC_APP.applyPermissionVisibility(p);$("#user-name").textContent=profile.nome;$("#user-profile").textContent=profile.perfil;$("#user-avatar").textContent=profile.nome.slice(0,1);await load()})().catch(e=>$("#users-loading").textContent=e.message);
