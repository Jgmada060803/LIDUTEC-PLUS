...
$("#user-sem-email").onchange=e=>{const semEmail=e.target.checked;$("#user-senha-field").hidden=!semEmail;$("#user-senha-field input").required=semEmail;$("#user-submit").textContent=semEmail?"Criar usuário":"Enviar convite"};
$("#user-form").onsubmit=async e=>{e.preventDefault();const b=e.submitter;const semEmail=$("#user-sem-email").checked;b.disabled=true;try{await invoke({action:"invite",...Object.fromEntries(new FormData(e.currentTarget))});e.currentTarget.reset();$("#user-senha-field").hidden=true;$("#user-submit").textContent="Enviar convite";$("#user-form-panel").hidden=true;await load();alert(semEmail?"Usuário criado com sucesso. Informe a senha provisória ao operador.":"Convite enviado com sucesso.")}catch(x){$("#user-message").textContent=x.message;$("#user-message").hidden=false}finally{b.disabled=false}};
