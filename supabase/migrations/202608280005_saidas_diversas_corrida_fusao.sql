...
-- Escória e lingote (saem do forno, abatem no saldo — igual transferência
-- de saída) e energia (só acompanha a corrida, não mexe no saldo em kg) —
-- pedido explícito, três caixas na tela de planejamento, valor único
-- editável quantas vezes precisar enquanto a corrida está aberta.
alter table public.corridas_fusao
  add column if not exists escoria_kg numeric,
  add column if not exists lingote_kg numeric,
  add column if not exists energia_kwh numeric;

create or replace function public.atualizar_saidas_diversas_corrida_fusao(
  p_corrida_id bigint, p_escoria_kg numeric, p_lingote_kg numeric, p_energia_kwh numeric
...
begin
  ...
  if coalesce(p_escoria_kg, 0) < 0 or coalesce(p_lingote_kg, 0) < 0 or coalesce(p_energia_kwh, 0) < 0 then
    raise exception 'Os valores não podem ser negativos.';
  end if;

  update public.corridas_fusao
  set escoria_kg = p_escoria_kg, lingote_kg = p_lingote_kg, energia_kwh = p_energia_kwh,
  ...
  values (p_corrida_id, auth.uid(), format('atualizou escória (%s kg), lingote (%s kg) e energia (%s kWh)',
    coalesce(p_escoria_kg, 0), coalesce(p_lingote_kg, 0), coalesce(p_energia_kwh, 0)));
end;
$$;

revoke all on function public.atualizar_saidas_diversas_corrida_fusao(bigint,numeric,numeric,numeric) from public, anon;
grant execute on function public.atualizar_saidas_diversas_corrida_fusao(bigint,numeric,numeric,numeric) to authenticated;

-- Escória/lingote entram como saída no cálculo do volume atual do forno,
-- igual transferência enviada pra outro forno.
...
returns numeric language sql stable set search_path = pg_catalog, public as $$
  select
    ...
    - coalesce((
      select sum(coalesce(corrida.escoria_kg, 0) + coalesce(corrida.lingote_kg, 0))
