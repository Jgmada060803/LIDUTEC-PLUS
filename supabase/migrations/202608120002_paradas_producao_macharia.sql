...
-- ==========================================================================
-- Paradas do apontamento de Macharia — mesmo padrão da Moldagem (Hora
-- início/fim, Setor responsável, Motivo, Observações), mas com
-- linha_maquina_id obrigatório: diferente da Moldagem (turno único, sem
-- distinção de linha), aqui cada máquina tem operador e apontamento
-- independentes, então a parada precisa saber de qual máquina é.
-- ==========================================================================
alter table public.turnos_producao_macharia
  add column if not exists rascunho_paradas jsonb not null default '[]'::jsonb;

create table if not exists public.paradas_producao_macharia (
  ...
  setor_responsavel_id bigint not null references public.setores_responsaveis_parada(id),
...
create index if not exists paradas_macharia_turno_idx
  on public.paradas_producao_macharia(turno_producao_id);
create index if not exists paradas_macharia_data_turno_idx
  on public.paradas_producao_macharia(data_operacional desc, turno);
create index if not exists paradas_macharia_linha_idx
  on public.paradas_producao_macharia(linha_maquina_id);

-- ==========================================================================
-- Triggers (funções genéricas já existentes, reaproveitadas)
-- ==========================================================================
drop trigger if exists validar_horario_parada_macharia on public.paradas_producao_macharia;
create trigger validar_horario_parada_macharia before insert or update of
  data_operacional,turno,inicio,fim on public.paradas_producao_macharia
for each row execute function public.validar_horario_turno_producao();
drop trigger if exists validar_area_parada_macharia on public.paradas_producao_macharia;
create trigger validar_area_parada_macharia before insert or update on public.paradas_producao_macharia
for each row execute function public.validar_acesso_producao_macharia();

-- ==========================================================================
-- RLS
-- ==========================================================================
alter table public.paradas_producao_macharia enable row level security;

drop policy if exists paradas_macharia_select on public.paradas_producao_macharia;
create policy paradas_macharia_select on public.paradas_producao_macharia for select to authenticated using(
...
drop policy if exists paradas_macharia_insert on public.paradas_producao_macharia;
create policy paradas_macharia_insert on public.paradas_producao_macharia for insert to authenticated with check(
  criado_por=auth.uid() and public.usuario_tem_permissao_producao_macharia('producao_macharia.lancar'));
drop policy if exists paradas_macharia_update on public.paradas_producao_macharia;
create policy paradas_macharia_update on public.paradas_producao_macharia for update to authenticated using(
...
grant select,insert,update on public.paradas_producao_macharia to authenticated;

grant usage,select on sequence public.paradas_producao_macharia_id_seq to authenticated;

-- ==========================================================================
-- RPCs (assinaturas mudam — precisam de drop antes do create or replace)
-- ==========================================================================
drop function if exists public.salvar_rascunho_turno_producao_macharia(date,text,jsonb,bigint);
...
begin
  ...
  if jsonb_array_length(coalesce(p_producoes,'[]'::jsonb))>500 or jsonb_array_length(coalesce(p_paradas,'[]'::jsonb))>200 then
...
$$;
revoke all on function public.salvar_rascunho_turno_producao_macharia(date,text,jsonb,jsonb,bigint) from public,anon;
grant execute on function public.salvar_rascunho_turno_producao_macharia(date,text,jsonb,jsonb,bigint) to authenticated;

drop function if exists public.fechar_turno_producao_macharia(date,text,jsonb,bigint);
...
$$;
revoke all on function public.fechar_turno_producao_macharia(date,text,jsonb,jsonb,bigint) from public,anon;
grant execute on function public.fechar_turno_producao_macharia(date,text,jsonb,jsonb,bigint) to authenticated;

drop function if exists public.editar_turno_producao_macharia(bigint,jsonb);
...
$$;
grant execute on function public.editar_turno_producao_macharia(bigint,jsonb,jsonb) to authenticated;
