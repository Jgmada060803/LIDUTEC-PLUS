-- Duas travas novas no apontamento do Vazamento:
-- 1) o início do vazamento de uma panela não pode ser antes da hora em
--    que ela foi tratada (retirada do Holding) — ordem física óbvia.
-- 2) ao fechar a panela que estava vazando (usando este início como fim
--    dela), esse horário não pode ser antes do início dela mesma — senão
--    fim < início, sem sentido. O lingotamento já tinha essa mesma trava
--    (iniciar_lingotamento_vazamento); faltava aqui no "Vazar".
...
begin
  ...
  -- não pode ser antes do início dela mesma.
  select * into v_aberta from public.panelas_holding
  where status = 'VAZADA' and hora_fim_vazamento is null for update;
  if found then
    if p_inicio < v_aberta.hora_inicio_vazamento then
      raise exception 'O início do vazamento não pode ser antes do início da panela que ainda está vazando (%).',
        to_char(v_aberta.hora_inicio_vazamento, 'HH24:MI');
    end if;
    update public.panelas_holding set hora_fim_vazamento = p_inicio, atualizado_em = now()
