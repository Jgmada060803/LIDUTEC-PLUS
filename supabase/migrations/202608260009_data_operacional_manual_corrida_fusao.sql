...
-- Data operacional passa a vir do formulário (junto com o turno, que já é
-- comum a todas as corridas) em vez de sempre usar a data de hoje do
-- servidor — necessário pra turno Noite virando o dia, ou lançamento
-- retroativo.
...
$$;
drop function if exists public.criar_corrida_fusao(bigint,text,integer,jsonb);
revoke all on function public.criar_corrida_fusao(bigint,text,date,integer,jsonb) from public,anon;
grant execute on function public.criar_corrida_fusao(bigint,text,date,integer,jsonb) to authenticated;
