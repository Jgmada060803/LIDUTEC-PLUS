-- O sequencial "Vn" (numeração da vazadora) deve reiniciar às 06:00, não à
-- meia-noite — o dia "operacional" da Fusão já segue essa regra em outras
-- partes do sistema (turnos.js: NOITE que cruza a virada conta pro dia
-- anterior até 06:00). O cálculo vem pronto do cliente (mesma lógica de
-- window.LIDUTEC_TURNOS.determineShift) em vez de refazer aqui com
-- extract(hour from ...), que dependeria do timezone da sessão do banco.
drop function if exists public.apontar_vazamento_panela(bigint, timestamptz, timestamptz, numeric, integer, integer, text, numeric);
...
grant execute on function public.apontar_vazamento_panela(bigint,timestamptz,timestamptz,numeric,integer,integer,text,numeric,date) to authenticated;
