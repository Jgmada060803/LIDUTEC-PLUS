...
-- "Caixa"/"Macho" sozinhos não identificam nada: o mesmo rótulo (ex. "CX1/
-- M1") se repete em dezenas de produtos sem nenhuma relação entre si. A
-- importação em lote comparava só caixa+macho pra decidir se uma linha era
-- "nova versão" de uma ficha já aprovada — o que fazia ela casar errado com
-- o macho de um produto completamente diferente. Agora a busca da ficha
-- vigente equivalente também exige que o conjunto de produtos vinculados
-- seja o mesmo.
...
begin
  ...
  loop
    ...
    from public.machos_macharia m
    where m.status='APROVADO'
      and upper(trim(m.caixa))=upper(trim(v_linha.caixa))
      and upper(trim(m.macho))=upper(trim(v_linha.macho))
      and coalesce((select array_agg(mp.produto_id order by mp.produto_id) from public.machos_macharia_produtos mp where mp.macho_id=m.id),'{}'::bigint[])
        = coalesce((
            select array_agg(produto.id order by produto.id)
