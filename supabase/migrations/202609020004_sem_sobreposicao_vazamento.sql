-- Só uma panela vaza por vez — bloqueia início/fim que se sobreponha ao
-- intervalo de outra panela já vazada (a fila em si já garante a ordem,
-- mas nada impedia digitar um horário que cruzasse com o de outra).
...
begin
  ...
  if p_fim <= p_inicio then raise exception 'O fim do vazamento precisa ser depois do início.'; end if;
  ...
  select sequencial, sequencial_vazamento into v_sobreposta
  from public.panelas_holding
  where status = 'VAZADA' and id <> p_panela_id
    and hora_inicio_vazamento < p_fim and hora_fim_vazamento > p_inicio
  limit 1;
  if found then
    raise exception 'Esse horário se sobrepõe ao vazamento da panela Nº % (V%) — só uma panela vaza por vez.',
      v_sobreposta.sequencial, v_sobreposta.sequencial_vazamento;
  end if;
