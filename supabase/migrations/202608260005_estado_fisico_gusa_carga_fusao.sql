...
-- Gusa pode entrar na carga sólido ou líquido — não é uma propriedade fixa
-- do material (o mesmo gusa cadastrado pode ser carregado de um jeito ou
-- de outro), é escolhido item a item na hora de montar a carga da corrida.
alter table public.corridas_fusao_carga_itens
  add column if not exists estado_fisico text check (estado_fisico in ('SOLIDO','LIQUIDO'));
